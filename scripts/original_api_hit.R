...
df <- if (http_status(response)$category == "Success") {
  result <- content(response, as = "text", encoding = "UTF-8") %>%
    safely(~ fromJSON(.x, flatten = TRUE))()
  
  if (!is.null(result$result)) result$result else tibble()
  
} else {
  log_info(glue("API failed with status {status_code(response)}: {http_status(response)$reason}"))
  tibble()
}
